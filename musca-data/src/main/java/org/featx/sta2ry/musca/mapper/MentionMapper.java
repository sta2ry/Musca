package org.featx.sta2ry.musca.mapper;

import org.apache.ibatis.annotations.*;
import org.featx.spec.model.PageRequest;
import org.featx.sta2ry.musca.criteria.MentionCriteria;
import org.featx.sta2ry.musca.entity.MentionEntity;

import java.util.List;

/**
 * Domain Module generated by coder as meta data below: Coder-Meta:
 * {
 *   "name": "Storage-Mapper",
 *    "data": {
 *       "org/featx/template/app": "com/juext/haldus/user", 
 *       "org.featx.template.app": "com.juext.haldus.user", 
 *       "DomainModule" : "${domain.name|camel}",
 *       "domainModule" : "${domain.name|camel}",
 *       "domain_module": "${domain.name|snake}"},
 *   "description": ""
 * }
 * @author Coder
 * @since 2020/4/12 13:49
 */

@Mapper
public interface MentionMapper {

    String COLUMUS = "id, code, name, type, status, sort, image_url, description, deleted, created_at, updated_at";

    @Insert({"insert into t_mention(code, name, type, status, sort, image_url, description) ",
            "value(#{entity.code}, #{entity.name}, #{entity.type}, #{entity.status}, #{entity.sort}, ",
            "#{entity.imageUrl}, #{entity.description})"})
    @SelectKey(statement = "SELECT LAST_INSERT_ID()", keyProperty = "entity.id", before = false, resultType = Long.class)
    int insert(@Param("entity") MentionEntity domainModuleEntity);

    @Update({"update t_mention set name = #{entity.name},",
            " type = #{entity.type}, status = #{entity.status},",
            " sort = #{entity.sort}, image_url = #{entity.imageUrl},",
            " description = #{entity.description}, ",
            " where code = #{entity.code} and deleted = 0"})
    int upsert(@Param("entity") MentionEntity domainModuleEntity);

    @Update({"<script>update t_mention <set> ",
            "<if test=\"entity.name != null and entity.name != ''\"> name = #{entity.name},</if>",
            "<if test=\"entity.type != null\"> type = #{entity.type},</if>",
            "<if test=\"entity.status != null\"> status = #{entity.status},</if>",
            "<if test=\"entity.sort != null\"> sort = #{entity.sort},</if>",
            "<if test=\"entity.imageUrl != null and entity.imageUrl != ''\"> image_url = #{entity.imageUrl}, </if>",
            "<if test=\"entity.description != null and entity.description != ''\"> description = #{entity.description}, </if>",
            "</set> where code = #{entity.code} and deleted = 0",
            "</script> "})
//    @SelectKey(statement = "select updated_at from t_mention where id = (SELECT LAST_INSERT_ID())")
    int update(@Param("entity") MentionEntity domainModuleEntity);

    @Update({"update t_mention set deleted = #{deleted} where code = #{code}"})
    int delete(@Param("code") String code, @Param("deleted") Boolean delete);

    @Select({"select ", COLUMUS, " from t_mention where deleted = 0 and code = #{code} limit 1"})
    MentionEntity selectByCode(@Param("code") String code);

    @Select({"<script>select ", COLUMUS, " from t_mention where deleted = 0 and code in ",
            "<foreach collection='codes' item='code' open='(' separator=',' close=')'>#{code}</foreach>",
            "</script>"})
    List<MentionEntity> selectByCodes(@Param("codes") List<String> codes);

    @Select({"select ", COLUMUS, " from t_mention where deleted = 0 ",
            "",
            "order by id limit #{page.offset}, #{page.size}"})
    List<MentionEntity> selectByPage(@Param("query") MentionCriteria criteria, @Param("page") PageRequest page);

    @Select({"select count(1) from t_mention where deleted = 0 ",
            ""})
    long countByQuery(@Param("query") MentionCriteria criteria);
}
